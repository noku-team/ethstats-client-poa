...
Socket = Primus.createSocket(webSocketOptions);
...
function Node() {
	...
	this.rpcHost = process.env.RPC_HOST || 'localhost';

	if (process.env.RPC_HOSTNAME) {
		var _t = this;
		var dns = require('dns');
		dns.lookup(process.env.RPC_HOSTNAME, function (err, result) {
			if (result && !err) {
				_t.rpcHost = result;
			}
			console.log("rpcHost", _t.rpcHost);
			_t.startWeb3Connection();
		})
	} else {
...
Node.prototype.startWeb3Connection = function () {
	...
	web3 = new Web3();
	web3.setProvider(new web3.providers.HttpProvider('http://' + (this.rpcHost) + ':' + (process.env.RPC_PORT || '8545')));
...
Node.prototype.startSocketConnection = function () {
	if (!this._socket) {
		...
		console.warn("ws server debug here", process.env.WS_SERVER);
		socket = new Socket(process.env.WS_SERVER || 'ws://localhost:3000', webSocketOptions);
